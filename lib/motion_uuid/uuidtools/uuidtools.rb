...
module UUIDTools
  ...
  class UUID
    ...
    def self.md5_create(namespace, name)
      return self.create_from_hash(RmDigest::MD5, namespace, name)
    end

    ##
    # Creates a UUID using the SHA1 hash.  (Version 5)
    def self.sha1_create(namespace, name)
      return self.create_from_hash(RmDigest::SHA1, namespace, name)
    end
  ...
  public
    ...
    def self.create_from_hash(hash_class, namespace, name)
      if hash_class == RmDigest::MD5
        version = 3
      elsif hash_class == RmDigest::SHA1
        version = 5
      else
        raise ArgumentError,
          "Expected RmDigest::SHA1 or RmDigest::MD5, got #{hash_class.name}."
      end
      hash = hash_class.hexdigest(namespace.raw + name) # We must combine here since there's no `update`
